		...
		if ( ! empty( $coauthors ) ) :
			...
			foreach ( $coauthors as $coauthor ) :
					...
					<li>
						...
						<span id="coauthor-readonly-<?php echo esc_attr( $count ); ?>" class="coauthor-tag">
	...
	function _filter_manage_posts_custom_column( $column_name ) {
		if ( $column_name == 'coauthors' ) {
			...
			foreach ( $authors as $author ) :
				...
				><?php echo esc_html( $author->display_name ); ?></a><?php echo esc_html( ( $count < count( $authors ) ) ? ',' : '' ); ?>
				<?php
				$count++;
			endforeach;
		}
	...
	function wp_notify_postauthor( $comment_id, $comment_type = '' ) {
		...
		foreach ( $coauthors as $author ) {
			...
			if ( 'comment' == $comment_type ) {
				...
				$notify_message .= sprintf( __( 'Website: %1$s (IP: %2$s , %3$s)' ), $comment->comment_author, $comment->comment_author_IP, $comment_author_domain ) . "\r\n";
				$notify_message .= sprintf( __( 'URL    : %s' ), $comment->comment_author_url ) . "\r\n";
				$notify_message .= __( 'Excerpt: ' ) . "\r\n" . esc_html( sprintf( '[...] %s [...]', $comment->comment_content ) ) . "\r\n\r\n";
				$notify_message .= __( 'You can see all pingbacks on this post here: ' ) . "\r\n";
				/* translators: 1: blog name, 2: post title */
				$subject = sprintf( __( '[%1$s] Pingback: "%2$s"' ), $blogname, $post->post_title );
			}
			$notify_message .= get_permalink( $comment->comment_post_ID ) . "#comments\r\n\r\n";
			$notify_message .= sprintf( __( 'Permalink: %s' ), get_permalink( $comment->comment_post_ID ) . '#comment-' . esc_attr( $comment_id ) ) . "\r\n";
			if ( EMPTY_TRASH_DAYS ) {
