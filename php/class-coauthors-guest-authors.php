...
class CoAuthors_Guest_Authors
{
	...
	function view_guest_authors_list() {
		...
		if ( isset( $_GET['action'], $_GET['id'], $_GET['_wpnonce'] ) && 'delete' == $_GET['action'] ) {
			...
			echo '<p>' . esc_html( __( 'You have specified this guest author for deletion:', 'co-authors-plus' ) ) . '</p>';
			echo '<p>#' . esc_html( $guest_author->ID . ': ' . esc_html( $guest_author->display_name ) ) . '</p>';
			echo '<p>' . esc_html( __( 'What should be done with posts assigned to this guest author?', 'co-authors-plus' ) ) . '</p>';
			echo '<p class="description">' . esc_html( __( "Note: If you'd like to delete the guest author and all of their posts, you should delete their posts first and then come back to delete the guest author.", 'co-authors-plus' ) ) . '</p>';
			...
			echo '<input type="radio" id="reassign-another" name="reassign" class="reassign-option" value="reassign-another" />&nbsp;&nbsp;' . esc_html( __( 'Reassign to another co-author:', 'co-authors-plus' ) ) . '&nbsp;&nbsp;</label>';
			...
			echo '<input type="radio" id="remove-byline" class="reassign-option" name="reassign" value="remove-byline" />&nbsp;&nbsp;' . esc_html( __( 'Remove byline from posts (but leave each post in its current status).', 'co-authors-plus' ) );
			echo '</label></li>';
			echo '</ul></fieldset>';
			...
			echo '<a href="' . esc_attr( $add_new_link ) . '" class="add-new-h2">' . esc_html( __( 'Add New', 'co-authors-plus' ) ) . '</a>';
			echo '</h2>';
	...
	function metabox_manage_guest_author_slug() {
		...
		echo '<p><label>' . esc_html( __( 'WordPress User Mapping', 'co-authors-plus' ) ) . '</label> ';
	...
	function metabox_manage_guest_author_name() {
		...
		foreach ( $fields as $field ) {
			...
			switch ( $field['input'] ) {
				case 'checkbox':
					echo '<input type="checkbox" name="' . esc_attr( $pm_key ) . '"'. esc_html( checked( '1', $value, false ) ) .' value="1"/>';
				break;
				default:
	...
	function metabox_manage_guest_author_contact_info() {
		...
		foreach ( $fields as $field ) {
			...
			switch ( $field['input'] ) {
				case 'checkbox':
					echo '<input type="checkbox" name="' . esc_attr( $pm_key ) . '"'. esc_attr( checked( '1', $value, false ) ) .' value="1"/>';
				break;
				default:
					echo '<input type="'. esc_attr( $field['input'] ) .'" name="' . esc_attr( $pm_key ) . '" value="' . esc_attr( $value ) . '" class="regular-text" />';
			break;
